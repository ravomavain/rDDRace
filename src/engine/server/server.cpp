	...
	while(pCmdEnd) //while there is a full (\r*\n-terminated) line available
	{
		...
		*(pCmdEnd+1) = '\0';//we overwrote the leftmost \n or \r
		...
		else
		{
			...
			Console()->Print(IConsole::OUTPUT_LEVEL_ADDINFO, "server", aBuf);
			m_RconClientID = -1;
			Console()->ExecuteLine(pCmdStart, 4, -1);
		}
...
int CServer::Run()
{
	...
	Console()->RegisterPrintResponseCallback(ConsolePrintCallback, this);
...
void CServer::ConBan(IConsole::IResult *pResult, void *pUser, int ClientID)
{
	...
	if(net_addr_from_str(&Addr, pStr) == 0)
	{
		...
		for(int i=0;i<MAX_CLIENTS;i++)
		{
			...
			{
				if (ClientID != -1 && (((CServer *)pUser)->m_aClients[ClientID].m_Authed > 0) && ((CServer *)pUser)->m_aClients[ClientID].m_Authed <= ((CServer *)pUser)->m_aClients[i].m_Authed)
				{
